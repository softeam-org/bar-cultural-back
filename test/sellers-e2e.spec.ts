...
describe('Sellers (e2e)', () => {
  ...
  test('/seller (GET)', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(200)
      .expect((response) => {
        expect(response.body.map((seller: Seller) => seller.cpf)).toEqual(
          expect.arrayContaining([cpf1, cpf2]),
        );
      });
  });
  ...
  test('/seller/:id (GET)', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(400)
      .expect((response) => {
        expect(response.body.message).toEqual('CPF inválido.');
      });
  });
